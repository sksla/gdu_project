...
<style>
  ...
  .app_title:hover, .delete_file:hover{cursor: pointer;}
  .sign_area{
  	height: 70%;
  	width: 100%;
  }
...
<body>
    ...
    <div class="page-wrapper">
        ...
        <!-- ----------------------------- 실제 내용 작성 영역 ----------------------------- -->
          ...
          <!-- 페이지 내용 -->
         	<form id="appForm" method="post" action="${contextPath}/approval/modify.do" enctype="multipart/form-data">
         		<!-- 기존 결재자 번호 -->
         		<c:if test="${ not empty collaboratorList }">
         			<c:forEach var="collaborator" items="${ collaboratorList }">
		          	<input type="hidden" name="collaboratorNo" value="${ collaborator.approver.memNo }">
         			</c:forEach>
         		</c:if>
         		<c:if test="${ not empty approver }">
	          	<input type="hidden" name="approverNo" value="${ approver.approver.memNo }">
         		</c:if>
         		<c:if test="${ not empty receiver }">
	          	<input type="hidden" name="receiverNo" value="${ receiver.approver.memNo }">
         		</c:if>
          	<!-- *********** -->
          	<input type="hidden" id="status" name="status" value="${ not empty collaboratorList ? 10 : 20 }">
          	<input type="hidden" name="nowStatus" value="${ docInfo.status }">
          	<input type="hidden" id="docNo" name="docNo" value="${ docInfo.docNo }">
            <div class="mb-4" style="display: flex; justify-content: space-between;">
              <span>
              	<button onclick="return enrollAppForm(1);" class="btn btn-info me-1 mb-1 px-4 fs-4">
              		${ docInfo.status == 0 or  docInfo.status == 1 ? '기안하기' : '재기안' }
              	</button>
              	<c:if test="${ docInfo.status == 0 or docInfo.status == 1 }">
              		<button onclick="return enrollAppForm(2);" class="btn btn-light mx-2 mb-1 px-4 fs-4">
              			임시저장
              		</button>
              	</c:if>
              </span>
            ...
            <div class="form-with-tabs">
                  ...
                  <div class="row">
                    <div class="col-lg-6 mb-lg-0">
                      <div class="mb-4 row align-items-center">
                        ...
                        <div class="col-sm-9">
                        	<c:choose>
                        		<c:when test="${ docInfo.status eq 2 }">
                        			<input class="form-control" type="text" value="${ docInfo.appCategory }" readonly>
                        		</c:when>
                        		<c:otherwise>
		                          <select class="form-select" id="selectAppCategory" aria-label="Default select example">
		                            <c:forEach var="appCategory" items="${appCategories}">
		                           		<option value="${appCategory}" ${ appCategory == docInfo.appCategory ? 'selected' : '' }>${appCategory}</option>
		                           	</c:forEach>
		                          </select>
                        		</c:otherwise>
                        	</c:choose>
                    ...
                    <div class="col-lg-6">
                      <div class="mb-4 row align-items-center">
                        ...
                        <div class="col-sm-9">
	                      	<c:choose>
	                       		<c:when test="${ docInfo.status eq 2 }">
	                       			<input class="form-control" type="text" value="${ docInfo.appNo }" readonly>
	                       		</c:when>
	                       		<c:otherwise>
                    ...
                    <div class="col-lg-6 mb-lg-0">
                      <div class="row align-items-center">
                        ...
                        <div class="col-sm-9">
                        	<c:choose>
                        		<c:when test="${ docInfo.status eq 2 }">
                        			<input class="form-control" type="text" value="${ docInfo.prePeriod }" readonly>
                        		</c:when>
                        		<c:otherwise>
              ...
              <div class="card">
                <div class="card-body">
                  <div class="row">
                    ...
                    <div class="">
                      <input class="form-control" type="text" name="docTitle" value="${ docInfo.docTitle }" 
                      			id="example-text-input" placeholder="결재문서의 제목을 입력해주세요" ${ docInfo.status == 2 ? 'readonly' : '' }/>
                    </div>
                  </div>
                </div>
                ...
                <div class="card-body">
                  ...
                  <div>
                  	...
                  	<ul>
                       	...
                       	<span class="ti ti-x mx-2 delete_file" data-fileno="${ att.fileNo }"></span>
                      </li>
                      </c:forEach>
                    </ul>
                    </c:if>
	          ...
	          <c:if test="${ docInfo.status != 0 and  docInfo.status != 1 }">
	          <!-- 결재 코멘트 -->
    ...
    <script>
        ...
        // 기안 결재선에 현재날짜 표시
    		...
    		// 최초 보존기한 표시
    		$("select[name='prePeriod']>option").each(function(index, el){
    			if($(el).text() == "${docInfo.prePeriod}"){
        ...
        $.ajax({
       		url:"${contextPath}/approval/formList.do",
       		data:"appCategory=" + $("#selectAppCategory").val(),
       		type:"get",
       		success: function(list){
       			$("#selectAppForm").empty();
       			$("#selectAppForm").append('<option value="0" disabled selected>문서양식을 선택하세요.</option>');
       			for(const appForm of list){
       				let $newEl = $('<option value="' + appForm.appNo + '">' + appForm.appFormName + '</option>');
       				if(appForm.appFormName == "${docInfo.appNo}"){
       					$newEl.prop("selected", true);
       				}
       				$("#selectAppForm").append($newEl);
       			}
       		},
       		error: function(){
       			console.log("양식 목록 ajax 통신 실패");
	    	...
	    	let totalSize = 0;
				...
				$("#upfiles").change(function(evt){
					...
					let fileNames = "<b class='my-2'>새로운 첨부파일 목록</b>";
   			...
   			// 삭제하고자 하는 해당 첨부파일 번호를 form submit시 넘기기 위한 작업
				$(".delete_file").on("click", function(){
     			...
     			document.getElementById("appForm").append(inputEl);
